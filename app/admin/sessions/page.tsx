...
type Sesion = {
  ...
  arrival_status: 'on_time' | 'late' | 'too_late' | null;
...
export default function AdminSessionsPage() {
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/admin/sessions/list?${qs.toString()}`, {
        headers: { 'x-admin-password': adminPass }
      });
      const j = await r.json();
      if (!r.ok || !j.ok) {
        setAuthed(false);
        setMsg('❌ Contraseña incorrecta o error al listar sesiones.');
        setItems([]);
      } else {
        setAuthed(true);
        setItems(j.items || []);
        setMsg(`✅ ${j.items?.length || 0} sesiones encontradas.`);
      }
    } catch (e: any) {
      setMsg('❌ ' + (e?.message || 'Error consultando sesiones'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportCsv = async () => {
    try {
      ...
      if (group) qs.set('group', group);
      if (status) qs.set('status', status);
      const r = await fetch(`/api/admin/sessions/export?${qs.toString()}`, {
        headers: { 'x-admin-password': adminPass }
      });
      if (!r.ok) {
        setMsg('❌ No se pudo exportar CSV.');
        return;
      }
      ...
      a.href = url;
      a.download = `asistencias_${date}.csv`;
      ...
      setMsg('❌ ' + (e?.message || 'Error exportando CSV'));
    }
  };
      ...
      <section className="bg-white/5 border border-white/10 rounded-2xl p-6">
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[220px]">
            <label className="block text-sm mb-1 opacity-80">Contraseña admin</label>
          ...
          <button onClick={fetchSessions} disabled={!adminPass || loading} className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/10">
            Consultar
          </button>
          <button onClick={exportCsv} disabled={!authed || loading} className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/10">
            Exportar CSV
          </button>
        </div>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(s => {
                ...
                const lleg = s.arrival_status
                  ? (s.arrival_status === 'on_time' ? 'A tiempo' : s.arrival_status === 'late' ? `Tarde (+${s.arrival_delay_min}m)` : 'Sin registro')
                  : '—';
                const est =
                  s.status === 'in_progress' ? 'En curso' :
                  s.status === 'finished' ? 'Finalizada' :
                  s.status === 'blocked' ? 'Bloqueada' : 'No iniciada';
              ...
              {items.length === 0 && (
                <tr><td className="py-3 opacity-70" colSpan={7}>Sin resultados para los filtros.</td></tr>
              )}
            </tbody>
          </table>
        </div>

        {/* Paneles de asistencia (lazy) */}
        <div className="mt-4 space-y-3">
          {items.filter(s => open === s.session_code).map(s => (
            <AsistenciaPanel key={s.session_code} sessionCode={s.session_code} />
          ))}
        </div>
      </section>
    </main>
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-4">
      ...
      {!loading && (rows?.length ?? 0) === 0 && (
        <div className="text-xs opacity-70">Sin registros de asistencia aún.</div>
      )}
