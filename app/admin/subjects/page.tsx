...
export default function SubjectsPage(){
  ...
  async function load(){ setErr(null);
    try{ const r=await fetch("/api/admin/subjects",{cache:"no-store"});
  ...
  return (<div>
    ...
    <table className="w-full text-sm">
      <thead className="text-left opacity-70 border-b"><tr><th className="py-2">ID</th><th>Código</th><th>Nombre</th><th>Creado</th><th/></tr></thead>
      <tbody>
        {items.map(s=>(
          <tr key={s.id} className="border-b">
            <td className="py-2">{s.id}</td><td>{s.code}</td><td>{s.name}</td>
            <td>{new Date(s.createdAt).toLocaleString()}</td>
            <td><button className="text-red-600" onClick={()=>remove(s.id)} disabled={loading}>Borrar</button></td>
          </tr>
        ))}
